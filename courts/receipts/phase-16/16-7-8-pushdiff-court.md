...
in-place, so no allocation or ownership changes) and mirrors the probe's
escaping byte for byte, including the SAX2 attribute `(value, valueEnd)` length
convention. `rc` is not a driver return value: `xmlParseChunk` returns `errNo`
when the document is not well-formed and 0 otherwise, and the same expression
is evaluated from the state the driver left behind.

The corpus is COMMITTED (`courts/suites/phase16/shadow-corpus`, 8 documents)
...
`inputNr`, `nameNr`, and the ordered SAX events **with payload and
segmentation** — `startDocument`/`endDocument`, start/end element with
namespace and attribute detail, `characters` (length AND bytes), `comment`,
`cdata`, `pi`, `reference`, `ignorableWhitespace`.

Two documented exclusions: structured diagnostic records (error occurrence and
code are already compared through `rc`/`errNo`; the text is the existing
courts' surface), and the `p=` field (see the finding below).

Plans: the deterministic probe subset `b1 b2 b3 b5 b257 Cb1 b1z2` for the
small documents (`b257` on a 4-byte document IS "whole document in one
non-final chunk"; the `C` prefix is the constructor-initial chunk; `z2` injects
zero-length calls), and `b1024 b4096` for the 4 KiB ones. 8 documents, 46
cells.

### Result (candidate `dabce012` + this step)

```
shadow: 46 cells, 39 match, 7 diverge
```

The 39 matches are exact, per call, including event segmentation and payload.
The 7 divergences are ALL the same document — `shadow-utf16trunc.xml`, on every
plan — and they are the encoder-flush cell this court was built to expose:

```text
oracle: < FINAL 0 rc=81 err=81 wf=0 in=-1 l=1 col=9 i=1 n=0
        (dom=8 code=81 "Invalid bytes in character encoding")
driver: < FINAL 0 rc=0  err=0  wf=1 in=-1 l=1 col=9 i=1 n=0
```

That is `xmlParserCheckEOF`'s `source_truncated` half, which
`pushdrive::check_eof` already documents as unimplemented — now measured
instead of assumed, with the oracle's exact codes (domain 8, code 81,
`endDocument` fired after the error). Fixing it is the next slice's first task.

### A second finding: the physical cursor is REBASED above 4096 bytes

Four cells (the two 4 KiB documents at FINAL) report `p` — upstream's
`cur - base` — as **80**, while the driver's absolute stream offset is 4103 /
4105:
...
This is the "absolute vs physical cursor" distinction, now with numbers.
...
`p` is therefore REPORTED but not asserted: the court-only driver does not
publish `ctxt->input` at all yet, so `p` is not a driver surface. It becomes
assertable once the driver publishes the input — and modelling the rebasing is
a prerequisite for that, not an optional extra.
...
1. Stateful push driver: park the parser (phase, element frames, tokenizer
   ...
   1c. ~~An oracle-facing SHADOW COURT.~~ DONE (step 6: 46 cells, 39 exact
       matches, 7 divergences all in the encoder-flush document; the physical
       cursor's >4096-byte rebasing measured).
   1d. Encoder-flush parity: implement `xmlParserCheckEOF`'s
       `source_truncated` half in `check_eof` from the observed oracle codes
       (domain 8, code 81, with `endDocument` after the error), and drive the
       shadow court to 46/46.
   1e. Publish `ctxt->input` from the driver and model the buffer rebasing so
       `p` becomes assertable, then extend the shadow court to the long
       documents' cursor field.
