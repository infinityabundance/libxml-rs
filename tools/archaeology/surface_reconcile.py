...
PROFILES = {
    "libxml2": {"version": "2.15.3", "system_dso": "/usr/lib/libxml2.so.16",
                "cand_dso": os.path.join(ROOT, "target", "debug", "libxml2.so.16"),
                "headers_dir": "/usr/include/libxml2/libxml"},
    "libxslt": {"version": "1.1.45", "system_dso": "/usr/lib/libxslt.so.1",
                "cand_dso": os.path.join(ROOT, "target", "debug", "libxslt.so.1"),
                "headers_dir": "/usr/include/libxslt"},
}

# The candidate is one cdylib installed under the libxml2/libxslt/libexslt
# SONAMEs (see build.rs); when a SONAME symlink is missing from target/debug
# the DSO comparison falls back to the raw cdylib so the reconciliation never
# silently drops the candidate side.
def cand_dso(project):
    p = PROFILES[project]["cand_dso"]
    if os.path.exists(p):
        return p
    fallback = os.path.join(ROOT, "target", "debug", "liblibxml_rs.so")
    print(f"  [surface_reconcile] {p} missing; using {fallback}")
    return fallback
...
def reconcile(project):
    ...
    cand_funcs, cand_objs = dso_symbols(cand_dso(project)) or (set(), set())
...
def write_markdown(out):
    """Generated Markdown view of atlas/SURFACE_RECONCILIATION.json.

    Every total is recomputed from the JSON — this file is never hand-edited.
    """
    lines = [
    ...
    for proj, r in out["projects"].items():
        if "error" in r:
            lines += [f"## {proj}", "", f"error: {r['error']}", ""]
            continue
        s = r["sources"]
        lines += [
            f"## {proj} {r['version']}",
            ...
            "| source | functions |",
            "|---|---|",
            f"| Doxygen inventory | {s['doxygen_functions']} |",
            f"| Clang AST atlas | {s['clang_ast_functions']} |",
            f"| System DSO (`nm -D`) | {s['system_dso_functions']} |",
            f"| Candidate DSO | {s['candidate_dso_functions']} |",
            f"| Installed headers | {s['installed_headers']} |",
            "",
            "### Function agreement",
            "",
            f"- in both Doxygen and AST: {len(r['functions']['in_both_dox_ast'])}",
            f"- Doxygen-only: {len(r['functions']['doxygen_only'])}",
            f"- AST-only: {len(r['functions']['ast_only'])}",
            "",
            "### Global (data) agreement",
            "",
            f"- in both Doxygen and AST: {len(r['globals']['in_both_dox_ast'])}",
            f"- Doxygen-only: {len(r['globals']['doxygen_only'])}",
            f"- AST-only: {len(r['globals']['ast_only'])}",
            "",
            "### Typedef / record agreement",
            "",
            f"- typedefs in both: {len(r['typedefs']['in_both'])}",
            f"  (Doxygen-only {len(r['typedefs']['doxygen_only'])}, "
            f"AST-only {len(r['typedefs']['ast_only'])})",
            f"- records in both: {len(r['records']['in_both'])}",
            f"  (Doxygen-only {len(r['records']['doxygen_only'])}, "
            f"AST-only {len(r['records']['ast_only'])})",
            "",
            "### DSO gap analysis",
            "",
            f"- system DSO functions not in Doxygen: "
            f"{len(r['dsos']['system_dso_functions_not_in_dox'])}",
            f"- candidate missing vs system: {len(r['dsos']['candidate_missing_vs_system'])}",
            f"- candidate extra vs system: {len(r['dsos']['candidate_extra_vs_system'])}",
            f"- candidate globals missing vs system: "
            f"{len(r['dsos']['candidate_globals_missing_vs_system'])}",
            "",
            "### Header agreement",
            "",
            f"- Doxygen headers: {len(r['headers']['doxygen_headers'])}",
            f"- installed headers: {len(r['headers']['installed_headers'])}",
            f"- Doxygen-only: {len(r['headers']['dox_not_installed'])}",
            f"- installed-only: {len(r['headers']['installed_not_dox'])}",
            "",
        ]
    with open(os.path.join(ATLAS, "SURFACE_RECONCILIATION.md"), "w") as f:
        f.write("\n".join(lines) + "\n")
