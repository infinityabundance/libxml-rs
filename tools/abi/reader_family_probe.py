#!/usr/bin/env python3
"""READER-001 — differential court for the xmlTextReader* family additions
(11.1-I reader closure).

Compiles courts/suites/data-abi/reader-family-probe.c against the system
...
Usage:
    python3 tools/abi/reader_family_probe.py
"""
...
PROBE = os.path.join(ROOT, "courts", "suites", "data-abi", "reader-family-probe.c")
...
def main():
    ...
    rc, _, err = run(["gcc", "-std=c11", "-w", "-o", os.path.join(ROOT, "target", "rd-oracle"),
                      PROBE, "-I", "/usr/include/libxml2", "-lxml2"])
    if rc != 0:
        print("ORACLE COMPILE FAILED:\n" + err[-2000:])
        return 1
    rc, o_out, err = run([os.path.join(ROOT, "target", "rd-oracle")])
    if rc != 0:
        print("ORACLE RUN FAILED:\n" + err[-2000:])
        return 1
    rc, _, err = run(["gcc", "-std=c11", "-w", "-o", os.path.join(ROOT, "target", "rd-cand"),
    ...
    if rc != 0:
        ...
        return 1
    rc, c_out, err = run([os.path.join(ROOT, "target", "rd-cand")])
    if rc != 0:
    ...
    receipt = {
        ...
        "phase": "11.1-I",
        "timestamp": ts,
        "probe": "courts/suites/data-abi/reader-family-probe.c",
    ...
    rpath = os.path.join(RECEIPTS, "reader-family-" + ts.replace(":", "").replace("-", "") + ".json")
