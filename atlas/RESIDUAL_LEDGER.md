...
### R-000179: Versioned-distro binary contract gap: the executed oracle (system 2.15.3) is UNVERSIONED, so the candidate core is unversioned; distro binaries that require the upstream LIBXML2_2.x named-version nodes (libxml2-2.13.5 libxml2.syms chain, e.g. Debian-built consumers) are not yet satisfied (FIXED)

- **Status:** FIXED (None, Phase 12)
...
- **Fix:** Phase 14.30 (R-000179 FIXED): a VERSIONED-PROFILE artifact now satisfies the distro .2 binary contract without touching the executed-oracle-parity core. tools/packaging/versioned-profile.sh links target/debug/versioned/libxml2.so.2.13.9 (SONAME libxml2.so.2 — the distro SONAME) from the same staticlib with tools/packaging/libxml2-versioned.syms, a version script generated by tools/packaging/versioned-profile-gen.py from the AUTHORITATIVE distro DSO's (/usr/lib/libxml2.so.2.13.9) .gnu.version tables: every candidate-defined symbol carries its exact upstream LIBXML2_2.x introduction node (43 nodes incl. the LIBXML2_2.4.30 base), following the upstream libxml2.syms inheritance pattern; 2.15-era exports the distro never versioned sit in a terminal LIBXML2_2.15.0 node (local: * hides internal leaks). The primary libxml2.so.16 core stays unversioned — exactly the executed 2.15.3 oracle — so every executed court is unchanged. The ELF-VERSIONING court gained six versioned-profile cases (guarded on the distro oracle's presence): node-graph, per-symbol node parity over the shared surface (one-directional: the distro's 2.13-only SAX1-era exports are absent from any 2.15-surface provider), a genuine distro-versioned consumer (linked against the .2 SONAME so DT_VERNEED records LIBXML2_2.x requirements) running byte-identical on the profile with zero ld.so version warnings, and ldd resolution into the profile.
...
- **Evidence:** ['tools/packaging/versioned-profile.sh', 'tools/packaging/versioned-profile-gen.py', 'tools/packaging/libxml2-versioned.syms', 'tools/packaging/linker-wrapper.sh', 'courts/suites/phase12/elf-versioning/court-runner.sh', 'courts/suites/phase12/elf-versioning/distro-versioned-consumer.c', 'courts/receipts/phase-12/elf-versioning-*.json']
- **Classification:** VERSION_DIFFERENCE
- **History:** OPEN 2026-08-31 (discovered in the first Phase-12 iteration: versioning the core with the upstream LIBXML2_2.x chain produced ld.so 'no version information available' warnings on every RUNPATH-resolved oracle-linked binary (the executed oracle is unversioned); reverted to the unversioned core and documented the bounded gap for non-executed distro binaries); FIXED 2026-09-05 (Phase 14.30: built the versioned-profile artifact (libxml2.so.2 SONAME) with the exact LIBXML2_2.x node graph derived from the host's real distro libxml2.so.2.13.9 .gnu.version tables, committed as libxml2-versioned.syms. A distro-style consumer linked against the versioned .2 SONAME (DT_VERNEED LIBXML2_2.x) runs against the profile with ZERO ld.so 'no version information available' warnings and byte-identical output (modulo the inherent version string); the profile is generated automatically after each link (linker-wrapper). ELF-VERSIONING court: 20/20 PASS. The unversioned .16 core is untouched (executed-oracle parity preserved; php NTS+ZTS gates unaffected).)
