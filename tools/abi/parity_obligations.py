...
def main():
    ...
    ledger = {"schema": "parity-obligations-3",
              ...
              # time is recorded in the generate_all.py receipt
...
def write_markdown(ledger):
    """Generated Markdown view of atlas/PARITY_OBLIGATIONS.json (11.1-W).

    Every total is recomputed from the JSON — never hand-edited.
    """
    L = ["# Parity Obligations — generated by tools/abi/parity_obligations.py", "",
         "For every externally relevant function/data object exported by the oracle",
    ...
    for proj, p in ledger["projects"].items():
        L += [f"## {proj} — oracle `{p['oracle_dso']}`", "",
              f"oracle functions: {p['oracle_functions']}, oracle data: {p['oracle_data']}, "
              f"candidate functions: {p['candidate_functions']}", "",
        ...
        for k in ("MISSING", "STUB", "INTENTIONAL_NOOP", "IMPLEMENTED_UNVERIFIED",
            ...
            if v:
                L.append(f"| {k} | {v} |")
        missing = [e for e in p["obligations"]
                   if e["overall"] in ("MISSING", "DATA_MISSING")]
        if missing:
            L.append("")
            L.append(f"### Missing ({len(missing)})")
            for e in sorted(missing, key=lambda e: e["oracle_symbol"]):
                L.append(f"- {e['kind']} `{e['oracle_symbol']}`")
        L.append("")
    with open(os.path.join(ATLAS, "PARITY_OBLIGATIONS.md"), "w") as f:
        f.write("\n".join(L) + "\n")
