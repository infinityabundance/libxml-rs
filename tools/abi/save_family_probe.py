#!/usr/bin/env python3
"""SAVE-001 — differential court for the xmlSave*/xmlOutputBuffer* family
(11.1-I serialization closure).

Compiles courts/suites/data-abi/save-family-probe.c against the system
...
Usage:
    python3 tools/abi/save_family_probe.py
"""
...
PROBE = os.path.join(ROOT, "courts", "suites", "data-abi", "save-family-probe.c")
...
def main():
    ...
    rc, _, err = run(["gcc", "-std=c11", "-w", "-o", os.path.join(ROOT, "target", "sf-oracle"),
                      PROBE, "-I", "/usr/include/libxml2", "-lxml2"])
    if rc != 0:
        print("ORACLE COMPILE FAILED:\n" + err[-2000:])
        return 1
    rc, o_out, err = run([os.path.join(ROOT, "target", "sf-oracle")])
    if rc != 0:
        print("ORACLE RUN FAILED:\n" + err[-2000:])
        return 1
    rc, _, err = run(["gcc", "-std=c11", "-w", "-o", os.path.join(ROOT, "target", "sf-cand"),
    ...
    if rc != 0:
        ...
        return 1
    rc, c_out, err = run([os.path.join(ROOT, "target", "sf-cand")])
    if rc != 0:
    ...
    receipt = {
        ...
        "phase": "11.1-I",
        "timestamp": ts,
        "probe": "courts/suites/data-abi/save-family-probe.c",
    ...
    rpath = os.path.join(RECEIPTS, "save-family-" + ts.replace(":", "").replace("-", "") + ".json")
