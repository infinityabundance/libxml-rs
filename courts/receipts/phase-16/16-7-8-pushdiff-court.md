...
## Slice 1 (step 6) — the ORACLE-SHADOW court

The driver's own court compares it to the candidate's RECURSIVE parser and to
internal invariants. This step compares it to **libxml2 2.15.3 itself, per
call** — the weakness the previous review identified.

### Two producers, one grammar

| side | produced by |
|---|---|
| oracle | `pushdiff-probe.c` run by `pushdrive-shadow-run.sh` against the system libxml2 |
| driver | `pushshadow.rs`, driving the court-only driver over the same documents |

Both emit the probe's own record grammar, so the comparison is a line-sequence
comparison:

```text
== <doc> bytes=<n> mode=<mode>
> CALL <n> len=<n>            (or  > CALL <n> ZERO)
<event lines and canonical error records, in dispatch order>
< CALL <n> rc=<rc> err=<e> wf=<w> in=<i> p=<p> l=<l> col=<c> i=<i> n=<n>
> FINAL / < FINAL 0 ...   and   > REFEED / < REFEED 0 ...
```

`pushshadow.rs` installs its own recording SAX handler (zeroing the context's
in-place, so no allocation or ownership changes), **plus a structured-error
recorder**, so diagnostics enter the same ordered stream as the SAX events.
It mirrors the probe's escaping byte for byte, including the SAX2 attribute
`(value, valueEnd)` length convention. `rc` is not a driver return value:
`xmlParseChunk` returns `errNo` when the document is not well-formed and 0
otherwise, and the same expression is evaluated from the state the driver left
behind.

The corpus is COMMITTED (`courts/suites/phase16/shadow-corpus`, 9 documents)
and generated by `gen_shadow_corpus.py`; the launcher runs the generator first
only to prove it still reproduces those bytes, and fails on drift. One source
of truth for both sides.

### The compared projection

Asserted per call: `rc`, `errNo`, `wellFormed`, `instate`, line, column,
`nameNr`, the ordered SAX events **with payload and segmentation**
(`startDocument`/`endDocument`, start/end element with namespace and attribute
detail, `characters` with length AND bytes, `comment`, `cdata`, `pi`,
`reference`, `ignorableWhitespace`), and the **ordered structured diagnostics**
in canonical form (`dom`/`code`/`level`/`line`/`int1`/`int2`).

Diagnostics are canonicalised rather than dropped: dropping them would let
`endDocument` before `error 81` pass exactly as readily as `error 81` before
`endDocument`, since only the final `errNo` would distinguish them — and once
declarations and DTDs arrive a single parse path can emit several.

Two fields are measured but NOT asserted, because they are not driver surfaces
yet: `p` (`cur - base`, rebased by upstream above 4096 bytes) and `i`
(`inputNr`, currently synthetic on the driver side). Both are printed by the
report.

Plans: `b1 b2 b3 b5 b257 Cb1 b1z2 b1i Cb1i r9-2` for the small documents and
`b1024 b4096 b1024i r17-512` for the 4 KiB ones — including the constructor
prefix, zero-length injections, **inline final** (the terminating flag arriving
WITH the last real bytes) and deterministic-random splits. 9 documents, 78
cells.

### Result

```
shadow: 78 cells, 78 match, 0 diverge
```
...
1. **EOF encoder flush missing.** `shadow-utf16trunc.xml` (a well-formed
   UTF-16LE document followed by one stray source byte) showed the oracle
   emitting `error dom=8 code=81`, then `endDocument`, then `rc=81`, while the
   driver returned `rc=0 wf=1`. `check_eof` now performs
   `xmlParserCheckEOF`'s flush half: when the input was consumed completely but
   the decoder still holds an incomplete unit, it raises
   XML_ERR_INVALID_ENCODING at the stream end. The unread-bytes check stays
   first and the flush stays gated on `errNo == OK`, so a malformed document's
   own error still wins.

2. **A trap in that fix.** `xmlFinishDocument` dispatches `endDocument` WITHOUT
   consulting `disableSAX`, and the flush sets `disableSAX = 1` — so reusing
   the policy-bearing `sax_end_document()` would have silently DROPPED the
   event while still marking it fired and counting it. `finalize_end_document()`
   now dispatches unconditionally, matching `xmlFinishDocument`. This is
   invisible on the normal path (disableSAX is 0) and decisive here.

3. **Definite invalid units were not handled at all.** A second encoder
   document, `shadow-utf16invalid.xml` (a lone low surrogate), separated a
   pathway the first cell could not: a DEFINITE invalid unit fails
   `xmlParserInputBufferPush`, so `xmlParseChunk` reports
   XML_ERR_INVALID_ENCODING on THAT call before `xmlParseTryOrFinish` runs —
   none of the call's decoded content is parsed and no grammar event fires.
   `push_persistent` now checks `source_encoding_error()` before driving.
   Three details came straight from the oracle rather than from reading:

   - the state is NOT frozen at `START`: the encoding switch precedes the parse
     loop, so the error is reported with `instate == XML_PARSER_XML_DECL`
     (`in=17`);
   - the position is the input's at the START of the call (`col=1` for a
     one-chunk feed, `col=4` for a feed that had already consumed `<a>`), not
     the end of the decoded stream;
   - later calls must NOT re-raise it — upstream returns the recorded `errNo`
     as soon as `disableSAX` is set, before pushing. Without that guard the
     error appeared twice.

### A second finding: the physical cursor is REBASED above 4096 bytes

Eight cells (the two 4 KiB documents at FINAL, on every plan) report `p` —
upstream's `cur - base` — as **80**, while the driver's absolute stream offset
is 4103 / 4105:

```text
shadow-longtext.xml b1024: oracle p=80  driver p=4103
shadow-longattr.xml b1024: oracle p=80  driver p=4105
```

Upstream rebases the input buffer when `cur - base > 4096` (`xmlParserShrink`)
and again through `xmlBufUpdateInput` in `xmlParserCheckEOF`'s encoder flush;
line/column stay absolute (both sides report `col=4104`) while `p` does not.
Below 4096 bytes there is no rebasing and `p` agrees.

The Step 1e design follows from this: `InputBuffer` stays the absolute logical
stream authority (absolute cursor, decoder state, and `ParkedConstruct`
offsets, which are all in that unit); `_xmlParserInput` becomes a synchronized
PHYSICAL view — base/cur/end, `consumed` for bytes removed from the window,
line/col — refreshed after reallocation and advanced on an upstream-equivalent
shrink. The C pointer window must never become the authoritative parser
position, and any representation REPLACEMENT must invalidate the lexical
continuation.
...
1. Stateful push driver: park the parser (phase, element frames, tokenizer
   ...
   1c. ~~An oracle-facing SHADOW COURT.~~ DONE (steps 6/6b: 78 cells, 78 exact
       matches, zero known-red cells — the encoder-flush and
       definite-invalid document pairs are both green).
   1d. ~~Encoder-flush parity and definite-invalid ingress.~~ DONE (step 6b,
       including `xmlFinishDocument`'s unconditional `endDocument`).
   1e. Publish `ctxt->input` from the driver and model the buffer rebasing
       (absolute logical position in `InputBuffer`, synchronized physical
       window in `_xmlParserInput`, `consumed` for bytes dropped by a shrink),
       then assert `p` and a real `inputNr` and widen the shadow corpus with
       entity inputs, where `inputNr` cannot stay synthetic.
   1f. Declarations and PIs, with real `XML_PARSER_XML_DECL` handling; then
       comments and CDATA reuse the `lookup_string` machinery from 5b. Any
       representation REPLACEMENT (declared-codec transcoding rebuilds the
       buffer from byte zero) must invalidate the lexical continuation —
       `ParkedConstruct::checked` is an offset into the current
       materialization — and that needs its own deliberate test.
   1g. DOCTYPE / internal subset (upstream's internal-subset scanner also
       persists `checkIndex` plus a richer `endCheckState`), then entity
       references.
   1h. Only then flip WHOLE contexts from replay to persistent at a point
       selected BEFORE the first observable event — never mid-document — and
       immediately re-run this court + the full push court + PHP + lxml +
       Nokogiri + pushscale. Replay state starts being DELETED at that point,
       not merely bypassed.
