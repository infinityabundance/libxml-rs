...
CUSTODIAN_EXTENSIONS = {
    # Upstream source builds with --with-thread-alloc export accessor
    # FUNCTIONS xmlMallocFunc *__xmlMalloc(void) & co (globals.c, gated by
    # LIBXML_THREAD_ALLOC_ENABLED); source-built consumers (upstream headers,
    # e.g. the canonical source oracle's libxslt) reference these instead of
    # the data variables. The executed distro oracle hides them. The
    # candidate exports both planes (R-000176 single source of truth: each
    # accessor returns addr_of_mut! of the matching exported variable), a
    # drop-in superset over the executed oracle (Phase-12 DOCKER-SUBSTITUTION
    # court, source-profile oracle).
    "__xmlMalloc": "R-000176/Phase-12: LIBXML_THREAD_ALLOC_ENABLED accessor (upstream globals.c); executed distro oracle hides it; source-built consumers reference it",
    "__xmlMallocAtomic": "R-000176/Phase-12: LIBXML_THREAD_ALLOC_ENABLED accessor (upstream globals.c); source-profile export",
...
def staticlib_syms(archive):
    """Global-defined symbols of the staticlib: the FULL #[no_mangle] surface
    of the Rust implementation. The shipped DSOs apply the generated version
    script, so a freshly added export is localized (dropped from .dynsym) and
    invisible to nm_dyn until the map is regenerated — consulting the archive
    makes the generator fail-closed (a new #[no_mangle] export can never be
    silently hidden or silently leaked). Rust-mangled internals (_ZN../_R..),
    compiler glue and non-C names are filtered out."""
    skip_prefixes = ("_ZN", "_R", "__rust_", "rust_eh", "__data_start",
                     "__dso_handle", "__bss_start", "___chkstk", "_init",
                     "_fini", "_edata", "_end")
...
def generate_xml_syms(names, node_map, node_chain, path):
    """Exact unversioned export map for libxml2.so.16 (Phase 12). The
    executed oracle (system 2.15.3) is UNVERSIONED: upstream 2.15 removed
    libxml2.syms, and versioning the core would make every oracle-linked
    consumer emit ld.so "no version information available" warnings when
    the DSO resolves via RUNPATH — an observable substitution difference.
    The upstream LIBXML2_2.x named-version chain (libxml2-2.13.5
    libxml2.syms) is therefore NOT replicated on the executed-oracle core;
    the versioned-distro binary contract (Debian-built consumers requiring
    LIBXML2_2.x nodes) is a documented bounded gap (R-000179). The export
    map still hides every INTERNAL_LEAK (local: *)."""
    funcs = sorted(n for n in names if names[n]["kind"] in ("T", "W", "i", "I"))
    data = sorted(n for n in names if names[n]["kind"] not in ("T", "W", "i", "I"))
    lines = ["# Generated by tools/phase12/export_surface.py — the exact\n",
             "# dispositioned export surface of libxml2.so.16 (1713 symbols: \n",
             "# CURRENT_ORACLE_EXPORT + CUSTODIAN_EXTENSION + \n",
             "# HISTORICAL_COMPAT_EXPORT), UNVERSIONED exactly like the executed\n",
             "# oracle (system 2.15.3). INTERNAL_LEAK symbols are hidden (local:\n",
             "# *). The upstream LIBXML2_2.x named-version chain is a documented\n",
             "# bounded gap (R-000179): the executed oracle has no version\n",
             "# definitions, and adding them would emit ld.so warnings for\n",
             "# oracle-linked consumers.\n",
             "{\n  global:\n"]
    for n in funcs:
        lines.append(f"    {n};\n")
    for n in data:
...
def main():
    ...
    # Candidate surface = current DSO exports UNION the staticlib's full
    # #[no_mangle] surface. The DSO alone is fail-open: the generated version
    # script localizes new exports before nm_dyn can see them (Phase-12
    # `__xml*` accessors are the concrete case), so a fresh export would be
    # silently hidden. The union makes every #[no_mangle] symbol accountable:
    # non-INTERNAL_LEAK symbols are added to the shipped map.
    staticlib = os.path.join(ROOT, "target", "debug", "liblibxml_rs.a")
    sl = staticlib_syms(staticlib) if os.path.exists(staticlib) else {}
    ...
    ledger = {"schema": "export-surface-disposition-1",
              ...
              "note": "shipped shared-DSO surface = executed-oracle surface + "
                       ...
                       "data compat + LIBXML_THREAD_ALLOC_ENABLED accessors); "
                       "INTERNAL_LEAK/HISTORICAL_COMPAT exports are hidden by "
                       "the generated version scripts; the candidate surface "
                       "is the DSO dynsym UNION the staticlib #[no_mangle] "
                       "surface so new exports cannot be silently hidden",
              "projects": {}}
    shipped_ok = True
    for project in ("libxml2", "libxslt", "libexslt"):
        oracle = nm_dyn(ORACLE[project])
        cand = nm_dyn(CANDIDATE[project])
        # staticlib symbols relevant to this project's DSO: everything except
        # the other projects' foreign-prefix surface (xslt*/exslt* belong to
        # their own facades) and the INTERNAL_LEAK Rust machinery — classified
        # below, hidden if leak.
        cand_full = dict(cand)
        # merge the staticlib #[no_mangle] surface relevant to THIS project's
        # DSO (the core carries all three projects; the facades carry their
        # own prefix). Existing DSO kinds win over archive kinds.
        prefixes = {"libxml2": ("",), "libxslt": ("xslt", "xsl"),
                    "libexslt": ("exslt",)}[project]
        for name, kind in sl.items():
            if name in cand_full:
                continue
        ...
        ledger["projects"][project] = {
            ...
            "candidate_surface_union": len(cand_full),
            "counts": counts,
            "symbols": rows,
        }
        ...
        for project in ("libxml2", "libxslt", "libexslt"):
            ...
            if project in ("libxslt", "libxml2"):
                ...
                if project == "libxslt":
                    ...
                    # libxml2: the executed oracle is unversioned; the
                    # candidate core must be too (R-000179: the upstream
                    # LIBXML2_2.x named-version chain is a documented
                    # bounded gap for non-executed distro binaries).
                    odefs, oneeds = read_version_info(ORACLE["libxml2"])
                    cdefs, cneeds = read_version_info(CANDIDATE["libxml2"])
                    if odefs != cdefs:
                        shipped_ok = False
                        print(f"  libxml2 version-definition mismatch: "
                              f"oracle={odefs} candidate={cdefs}")
                    else:
                        print(f"  libxml2 version definitions match "
                              f"(none on either side: {len(odefs)})")
            else:
                odefs, oneeds = read_version_info(ORACLE[project])
                cdefs, cneeds = read_version_info(CANDIDATE[project])
                if odefs != cdefs:
